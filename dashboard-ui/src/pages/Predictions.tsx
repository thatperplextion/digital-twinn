  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-slate-400 mt-1">AI-powered predictive intelligence for digital twins</p>
      </div>
        ...
        <div className="glass rounded-xl p-4 border border-slate-700/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-white">{stats.validated}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 glass rounded-2xl p-6 border border-slate-700/50">
          <h3 className="font-semibold text-white mb-4">Prediction Accuracy Trend</h3>
          <GradientAreaChart
            data={accuracyData}
            dataKey="accuracy"
            color="#22c55e"
        ...
        <div className="glass rounded-2xl p-6 border border-slate-700/50">
          ...
          <PieChartComponent data={predictionTypes} height={200} showLabels={false} />
      ...
      <div className="flex gap-4">
        ...
        >
          ...
          <option value="NEXT_ACTION" className="bg-slate-800">Next Action</option>
          <option value="STATE_TRANSITION" className="bg-slate-800">State Transition</option>
          <option value="RISK_FORECAST" className="bg-slate-800">Risk Forecast</option>
          <option value="FAILURE_PROBABILITY" className="bg-slate-800">Failure Probability</option>
        </select>
        <select
          value={statusFilter}
        ...
        >
          ...
          <option value="VALIDATED" className="bg-slate-800">Validated</option>
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className={clsx('px-2 py-1 rounded text-xs font-medium', typeColors[pred.type])}>
                    {pred.type.replace('_', ' ')}
                  </span>
                  <span className={clsx('px-2 py-1 rounded text-xs font-medium', statusColors[pred.status])}>
                    {pred.status}
                  </span>
                  <span className="text-xs text-slate-500">{pred.id}</span>
                  <span className="text-xs text-slate-500">•</span>
                  <span className="text-xs text-slate-500">Twin: {pred.twinId}</span>
                </div>
                <h4 className="font-medium text-white mt-2">{pred.description}</h4>
                <p className="text-sm text-slate-400 mt-1">
                  Outcome: <span className="text-white">{pred.predictedOutcome}</span>
                </p>
              </div>
              <div className="text-right ml-4">
                <p className="text-3xl font-bold text-white">{(pred.probability * 100).toFixed(0)}%</p>
            ...
            <div className="mt-4 pt-4 border-t border-slate-700/50 grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {pred.explanation.keyFactors.slice(0, 3).map((factor, i) => (
                    <span
                      key={i}
                      className={clsx(
                        'px-2 py-1 rounded text-xs',
                        factor.direction === 'POSITIVE' ? 'bg-green-500/20 text-green-400' :
                        factor.direction === 'NEGATIVE' ? 'bg-red-500/20 text-red-400' :
                        'bg-slate-500/20 text-slate-400'
                      )}
                    >
                      {factor.name}: {String(factor.value)}
                    </span>
                  ))}
                </div>
              </div>
              <div className="text-sm text-slate-500 md:text-right space-y-1">
                <p>Model: {pred.modelUsed} v{pred.modelVersion}</p>
                <p>Confidence: {pred.explanation.confidence}</p>
                <p>Expires: {new Date(pred.expiresAt).toLocaleString()}</p>
              </div>
            </div>
          </motion.div>
        ))}
